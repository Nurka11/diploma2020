...
def get_page_data(html):
    ...
    for i in soup.findAll('a', {"class": 'j_product_link'}):
        res = json.loads(i.get('data-seo'))
        res.update({'link': i.get('href')})
        mechta.append({
            'name': res['name'],
            'categoryName': res['category'],
            'price': res['price'],
            'url': 'https://www.sulpak.kz/{}'.format(res['link']),
            'shop': 'Mechta'
        })
    return mechta
...
def mechta(url_target, page_count):
    pattern = url_target + '?PAGEN_2={}'
